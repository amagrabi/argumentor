...
async function getNewQuestion() {
    try {
        ...
        let query = selectedCategories.length > 0 ? `?categories=${selectedCategories.join(',')}` : '';
...
document.getElementById('submitAnswer').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      document.getElementById('errorMessage').textContent = errorData.error || 'Submission failed';
      return;
    }
    ...
    document.getElementById('miniXpBar').firstElementChild.style.width = data.level_info.progress_percent + '%';
  } catch (error) {
    console.error('Error submitting answer:', error);
    document.getElementById('errorMessage').textContent = 'Submission failed - server error';
  }
});
          ...
          groups[category].forEach(question => {
            ...
            item.addEventListener('click', () => {
              ...
              .then(selected => {
                ...
                typeWriter(document.getElementById('questionDescription'), selected.description, 15);
                document.getElementById('answerInput').value = '';
                document.getElementById('charCount').textContent = '200';
...
document.getElementById('questionSelectionOverlay').addEventListener('click', (e) => {
